...
export const googleWorkspaceProvisioningApi = {
  ...
  buscarDirectorio: (q) => api.get('/google-workspace-provisioning/directory/buscar', { params: { q } }),
  exportarResultados: async (jobId) => {
    ...
    const blob     = await res.blob();
    const filename = res.headers.get('Content-Disposition')?.match(/filename="?([^"]+)"?/)?.[1]
                     || `Pandora_Alta_Masiva_GoogleWorkspace_${new Date().toISOString().slice(0,10)}.xlsx`;
    const href = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = href; a.download = filename; a.click();
    URL.revokeObjectURL(href);
